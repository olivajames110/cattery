...
const SpotsAvailable = ({ parties, currentOccupancy, currentNumOfUpcomingReservations, currentNumOverdue }) => {
	...
	let checkifOverlap = () => {
		...
		let overLappingParties__true = sortArrayByKey(parties, 'isOverlap', true);
		let overLappingParties__false = sortArrayByKey(parties, 'isOverlap', false);
		let overLapOffset = 0;

		if (parties.length >= 1 && reservationList.length >= 1 && overLappingParties__false) {
			...
			sumOfOverLappingParties__false = overLappingParties__false.reduce(function(prev, cur) {
