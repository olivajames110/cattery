...
class Cattery extends Component {
  ...
  checkPartyStartTimes = () => {
    ...
    for (let i = 0; i < this.state.parties.length; i++) {
      ...
      if (
        party[i].times.start === this.state.times.currentTime &&
        party[i].rowNum !== 1
      ) {
        party[i].isUpcomingReservation = true;
  ...
  handleCheckReservation = (id, numOfNewPeople) => {
    ...
    let numOfUpcoming = filteredParty[0].isUpcomingReservation
  ...
  //returns object of start and end time -------------MOVE TO HELPERS-------------------
  handleGetTimes = () => {
  ...
  // Updates start/end times__by ID-------------MOVE TO HELPERS-------------------
  ...
  //Updates data__by ID-------------MOVE TO HELPERS-------------------
