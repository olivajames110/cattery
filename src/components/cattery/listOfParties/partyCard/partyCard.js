...
class PartyCard extends Component {
	...
	handleButtonClick = () => {
		if (!this.state.isReservation) {
			this.props.onClick_remove(this.props.party.id, this.props.party.numberInParty, this.state.timeRemaining);
	...
	render() {
		...
		return ( <
			...
			className = {
				`party-wrapper ${this.state.isReservation ? 'reservation' : ''}`
			} >
			<
			...
			span style = {
				{
					color: this.state.isPaid ? '#6fa037' : '#c1c0c0'
				}
			}
